    ...
    return (
      <div className="overflow-hidden rounded-[1.75rem] border border-[#dfe8e4] bg-[linear-gradient(165deg,#ffffff_0%,#f4faf7_100%)] px-6 py-14 text-center sm:px-10">
        ...
        <h2 className="mt-6 text-2xl font-semibold tracking-tight text-[#1a2e28]">
          {isImage ? "No gallery posts yet" : isProfile ? "No profiles yet" : "No classifieds in this view"}
        </h2>
        <p className="mx-auto mt-3 max-w-md text-sm leading-relaxed text-[#4a635c]">
          {isImage
            ...
            : isProfile
              ? "As profiles are published, this directory will turn into a stronger trust surface for people and brands."
        ...
        <div className="mt-8 flex flex-wrap items-center justify-center gap-3">
          <Button asChild className="rounded-full bg-[#2d5a4c] text-white hover:bg-[#23463c]">
            <Link href={isImage ? "/create/image" : isProfile ? "/create/profile" : "/create/classified"} className="inline-flex items-center gap-2">
              {isImage ? "Upload photos" : isProfile ? "Create profile" : "Post a deal"}
              <ArrowRight className="h-4 w-4" />
            </Link>
          </Button>
          <Button asChild variant="outline" className="rounded-full border-[#dfe8e4] bg-white">
            <Link href={isImage ? "/classifieds" : isProfile ? "/articles" : "/images"}>
              {isImage ? "Browse deals" : isProfile ? "Read articles" : "Open gallery"}
            </Link>
          </Button>
        </div>
      </div>
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-3 border-b border-[#dfe8e4] pb-5 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className={cn("text-xs font-semibold uppercase tracking-[0.22em]", ui.muted)}>
            {isImage ? "Gallery stream" : isProfile ? "Profile directory" : "Live board"}
          </p>
          <h2 className={cn("mt-1 text-2xl font-semibold tracking-tight", ui.heading)}>
            {isImage ? "Latest uploads" : isProfile ? "Featured people and brands" : "Fresh listings"}
          </h2>
        </div>
        <p className={cn("text-sm", ui.muted)}>
          Showing <span className={cn("font-semibold", ui.heading)}>{merged.length}</span>{" "}
          {merged.length === 1 ? "result" : "results"}
          {category && category !== "all" ? (
            <span>
              {" "}
              in <span className={cn("font-medium", ui.heading)}>{category.replace(/-/g, " ")}</span>
            </span>
          ) : null}
        </p>
      </div>
      ...
      {(isImage || isClassified || isProfile) && merged.length >= 8 ? (
